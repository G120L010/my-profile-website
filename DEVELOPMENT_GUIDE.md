...
| 2026.07.23 | **修復在線人數 (onlineVisitors) 只增不減、重複 ID 與行動端 visibilitychange 監聽機制** | [App.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/js/App.js)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | (1) 行動裝置關閉或切換分頁時 beforeunload/pagehide 未可靠觸發 down 導致雲端計數積累；(2) updateCount 中 lastCloudCount 與 Math.max 鎖定過往最高人數致只增不減；(3) sendLocalHeartbeat 領袖選舉中 myClientId 被重複加入 activeTabIds 陣列；(4) 未監聽 visibilitychange 事件以致手機 App 背景/鎖屏未自動休眠扣減。 | (1) 重構 initOnlinePresence，新增 visibilitychange 事件監聽，讓分頁於 hidden 狀態時自動執行離場扣減與休眠，切回 visible 時重新上線；(2) 修正 updateCount 邏輯，移除卡死歷史高值的鎖定機制，並加入以今日瀏覽人流為基準的數據合理上限保護；(3) 修正領袖選舉 activeTabIds 重複 ID 陣列 Bug；(4) 100% 恪守繁體中文純文字與無 Emoji 註解規範。 |
