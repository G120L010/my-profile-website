...
| 2026.07.23 | **以純 GET 模式重構在線人數 Presence 達成跨裝置實時動態同步與 0 控制台報錯** | [App.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/js/App.js)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | (1) 原 `sendBeacon` 發送 POST 請求引發 CounterAPI 回傳 404 (Not Found) 並阻擋 CORS；(2) 直接查詢未初始金鑰導致 HTTP 404 與 `net::ERR_FAILED`；(3) 單純使用 BroadcastChannel 時手機與電腦跨裝置無法同步人數；(4) `updateCount` 鎖定高值與領袖重複 ID 影響動態增減。 | (1) 重構 `initOnlinePresence`，改採純 GET 請求模式並在 init 時自動發送 `/presence/up` 建立 200 OK 跨域標頭，徹底消除 CORS 阻擋；(2) 改以 `fetch(..., { method: 'GET', keepalive: true })` 替代 `sendBeacon` 發送離線扣減，解決 POST 404 報錯；(3) 透過領袖選舉 (Leader Election) 15s 降頻輪詢，實現手機與電腦跨裝置連線時人數自動遞增、離線/背景休眠時自動遞減；(4) 100% 恪守繁體中文純文字與無 Emoji 註解規範。 |
