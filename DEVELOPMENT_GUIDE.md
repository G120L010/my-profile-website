...
| 2026.07.23 | **全站導入 vue-i18n 中英文雙語切換與持久化儲存機制** | [App.vue](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/App.vue)<br>[App.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/js/App.js)<br>[App.css](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/css/App.css)<br>[main.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/main.js)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | (1) 網頁原先僅支援單一繁體中文，缺乏英文版本以對應海外與跨國職缺投遞需求；(2) 打字機跑字內容原先寫死中文，切換語系時無法同步動態轉譯與重置打字進度；(3) `src/assets/js/App.js` 引進 i18n 配置檔時使用了 `../i18n/index.js` 相對路徑，少向上跨一級目錄導致 Vite 拋出 `Failed to resolve import` 錯訊。 | (1) 安裝並配置 `vue-i18n@10` 套件，建立 `src/locales/zh-TW.js` 與 `en.js` 雙語字典，實作 `src/i18n/index.js` 並於 `main.js` 註冊插件；(2) 將 `App.js` 中之引入路徑更正為 Vite 路徑別名 `@/i18n/index.js` 徹底消除解析報錯；(3) 於 App.js 實作 `switchLanguage` 函式將語言偏好寫入 `localStorage` (key: `han_language_pref`) 達到跨 session 記憶；(4) 重構 App.js 打字機邏輯，監聽 locale 變更自動清空進度並重新以目標語系打字；(5) 全面替換 App.vue 大外殼靜態標籤為 `$t(...)` 動態綁定。 |
| 2026.07.23 | **全平台懸浮語言切換按鈕重構移動至視窗左上角** | [App.css](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/css/App.css)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | 應使用者要求改善介面佈局：原先在右下角與主題切換按鈕、回到頂部按鈕放在同側容易造成直向按鈕過度堆疊，不論在手機或桌機瀏覽器皆影響視覺簡潔度。 | (1) 將 `.custom-lang-toggle-btn` 完全移動至視窗左上角，桌機版設定為 `top: 1.5rem; left: 1.5rem;`；(2) 行動端 (<=576px) 設定為 `top: 1rem; left: 1rem;` (44px*44px)；(3) 復原右下角回到頂部按鈕邊距為 `bottom: 6rem; right: 2rem;` (手機版 `bottom: 4.5rem; right: 1rem;`)；達成左上角獨立語言切換、右下角主題與置頂二重奏的最優佈局。 |
| 2026.07.23 | **擴充日文 (ja) 語系支援與三語系 (繁中/英/日) 循環切換與目標語言標籤** | [ja.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/locales/ja.js)<br>[i18n/index.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/i18n/index.js)<br>[App.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/js/App.js)<br>[App.vue](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/App.vue)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | (1) 應使用者要求擴充國際化支援，除繁中與英文外額外新增日文（日本語）選項；(2) 改善使用者體驗 (UX)：原本顯示「當前語言名稱」導致外國訪客在中文頁面無法辨識左上角為語系切換鍵。 | (1) 建立 `src/locales/ja.js` 日文翻譯字典；(2) 於 `src/i18n/index.js` 引入並註冊 `ja` 語系；(3) 重構 `App.js` 中的 `toggleLanguage` 與 `getFullText` 邏輯，實現 `zh-TW` -> `en` -> `ja` -> `zh-TW` 三語系順暢循環切換與日文打字機跑字；(4) 優化 `App.vue` 按鈕標籤顯示邏輯為「提示下一個目標語言」（中文頁面顯示 **EN**、英文頁面顯示 **にほんご**、日文頁面顯示 **繁中**），方便跨國訪客直覺切換。 |
| 2026.07.23 | **語言按鈕『にほんご』文字換行跑版修正與 CSS 強制單行重構** | [App.css](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/css/App.css)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | 當按鈕標籤切換為 4 個平假名 `にほんご` 時，因原 `font-size: 1.25rem` (20px) 過大，在正圓按鈕內文字無法單行容納而跳行拆成兩列或向上溢出跑版；光把按鈕寬度加大至 58px 無法根本解決字型過大問題。 | 重構 `.custom-lang-toggle-btn` CSS 樣式：(1) 加入 `span { white-space: nowrap !important; line-height: 1 !important; }` 徹底禁止文字折行；(2) 將桌機字體調為 `0.82rem`，手機端 (<=576px) 字體調為 `0.78rem` (約 12px) 配合 `letter-spacing: -0.3px`；(3) 使 `にほんご` 4 個字在 50px 正圓按鈕內 100% 水平單行完美置中不跑版。 |
