...
| 2026.07.23 | **行動端懸浮按鈕尺寸放大優化 (50px * 50px 強化觸控與視覺)** | [App.css](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/css/App.css)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | 手機/窄螢幕下（<=576px）原先將三顆按鈕微調縮小至 `44px`，但由於左上角語言按鈕標籤為 `にほんご`，且 `44px` 圓形在手機觸控時顯得略小不夠大氣與舒適。 | 於 `@media (max-width: 576px)` 重新優化按鈕尺寸：(1) 主題按鈕、語言按鈕與置頂按鈕統一放大為 `50px * 50px`；(2) 語言切換按鈕字體調為 `0.85rem` 確保 `にhonご` 清晰置中不擠壓；(3) 置頂按鈕底部距調為 `bottom: 4.8rem;`，大幅提升行動端點擊舒適度與視覺美感。 |
