...
| 2026.07.23 | **徹底消除 presence CORS 404 與 net::ERR_FAILED 報錯暨行動端休眠機制** | [App.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/js/App.js)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | (1) GitHub Pages 靜態環境對未創立/限流之 CounterAPI `/presence` 金鑰發送 HTTP 請求引發 CORS Policy 阻擋與 `net::ERR_FAILED` 報錯；(2) 原 `updateCount` 鎖定歷史高值致人數只增不減；(3) 領袖選舉重複 ID Bug；(4) 未監聽 `visibilitychange` 致 App 背景/鎖屏未休眠。 | (1) 徹底移除 `initOnlinePresence` 對外之 `presence` 請求，回歸乾淨安全的 BroadcastChannel 心跳、Storage 視窗事件與 localStorage 分頁註冊，達成 100% 零控制台錯訊；(2) 新增 `visibilitychange` 事件監聽，分頁於 hidden 狀態時自動執行休眠扣減，切回 visible 時恢復上線；(3) 修正 `updateCount` 移除去卡死高值機制；(4) 100% 恪守繁體中文純文字與無 Emoji 註解規範。 |
