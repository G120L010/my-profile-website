...
| 2026.07.23 | **人流計數器歸零邏輯修正：今日/本月/本年帶日期後綴 Key 實現週期性歸零** | [App.js](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/src/assets/js/App.js)<br>[DEVELOPMENT_GUIDE.md](file:///c:/Users/s1080/Desktop/JOHN/my-profile-website/DEVELOPMENT_GUIDE.md) | (1) 舊版 `today`/`month`/`year` 使用固定 API Key，伺服器端與本地快取均不會隨日期、月份、年份重置，導致四個數字永遠累積不歸零，今日瀏覽與本月瀏覽數字異常膨脹，邏輯嚴重錯誤；(2) 本地 localStorage 以單一 JSON 物件儲存四值，換日後仍會讀取到舊快取而繼續顯示過期的高值；(3) API 回傳值判斷使用 `Math.max(val, cached + 1)` 導致數值異常膨脹；(4) 總數計算使用 `yearCount + 10` 作為基線，人為虛增累計數字，邏輯不合理。 | (1) 將 `today`/`month`/`year` 的 API Key 與 localStorage 快取 Key 均帶入當前日期後綴（例如 `today_2026_07_23`、`month_2026_07`、`year_2026`），換日/月/年後自動使用全新計數器，真實模擬歸零效果；(2) 僅 `total_v15` 使用固定 Key，確保累積總數跨日期持續增加；(3) 將四個計數器的 localStorage 拆分為獨立 key 分別讀寫，各週期互不干擾；(4) API 成功時直接使用 `val`（移除強制 `max` 膨脹），API 失敗時以 `cachedVal + (isNewSession ? 1 : 0)` 作為合理 fallback；(5) 移除 `totalCount = Math.max(yearCount + 10, rawTotal)` 的人為加值，改為合理的 `Math.max(yearCount, rawTotal)`；(6) Session 判斷 Key 同步改為帶日期後綴，防呆換日後誤判為舊 session。 |
